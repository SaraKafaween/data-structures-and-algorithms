...
const count = (target, input) => {
  ...
  return newArr.filter((element)=> element === target).length;
};
...
const totalSum = (input) => {
  const totalySum = input.reduce((final, sumArray) => {
    return final += sumArray.reduce((res, num2) => {
      return res += num2;
    }, 0);
  }, 0);
  return totalySum;
...
let findMaleAndFemale = (data) => {
  ...
  const added = filtered.map((element)=> element.name);
