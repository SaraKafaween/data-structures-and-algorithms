...
const sortStarWarsCharacters = (starWarsArr) => {
  let newArr = starWarsArr.sort((item1, item2) => {
...
const joinArray = (arr) => {
  let newArr3 = arr.reduce((fst, snd) => {
    return fst + ' ' + snd;
  });
  return newArr3;
};
...
const wordsToCharList = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    result2.push(arr.substring(i, i + 1));
  }
  return result2;
};
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((element) => {
    for (let i = 0; i < 2; i++) {
      firstone = element.slice(element.indexOf(' ')+1);
      secondone = firstone.slice(firstone.indexOf(' ')+1);
    }
    result.push(secondone);
  });
  return result;
};
